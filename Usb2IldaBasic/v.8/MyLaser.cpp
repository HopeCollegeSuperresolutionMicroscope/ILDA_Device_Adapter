...
const char g_ILDARed637LaserDACChannelBit = 0x01;
const char g_ILDAGreen532LaserDACChannelBit = 0x02;
const char g_ILDABlue445LaserDACChannelBit = 0x03;
...
int ILDAMCP4271::SetVoltage(long double setVoltage)
{
	...
	//Command Byte
	data[0] = writeCmds_[ singleWrite ] | addressDACChannel_ & ~1;
...
ILDALaser::ILDALaser( BeamColor color, unsigned long resolution ) : ILDAMCP4271(),
initialized_(false),
busy_(false),
...
int ILDALaser::OnVoltage(MM::PropertyBase* pProp, MM::ActionType eAct)
{
   ...
   else if (eAct == MM::AfterSet)
   {
	  ...
	  currentVoltage = SetVoltage(currentVoltage);
	  LogMessageCode(currentVoltage, false);
...
ILDASystemShutter::ILDASystemShutter( unsigned long resolution ) : ILDAMCP4271(), 
initialized_(false), 
